...
static void *PREV(void* bp) {return (void*)(uint32_t)bp+1};

//1 word, 2-2 words, 4-7 words, etc up to 1024-infinity
...
void remSize(void* bp) {
    if (*(uint32_t)bp != NULL) {
        //next->prev = prev
        PUT(PREV(*(uint32_t)bp),PREV(bp));
    }
    if (PREV(bp) != NULL) {
        //prev->next = next
        PUT(PREV(*(uint32_t)bp+1),bp);
    }
...
static void *find_fit(uint32_t asize)
{
    ...
    void *temp = sizes[index];
    while (index < 9) {
        while (temp != NULL) {
            if ( asize < GET_SIZE(HDRP(temp)) ) {
                if (PREV(temp) != NULL) {
                    PUT(PREV(temp),(void*)*(char*)temp);
                } else {
                    sizes[index] = NULL;
                }
                return temp;
