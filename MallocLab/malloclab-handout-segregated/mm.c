...
static void* PREV(void* bp) {return bp;};
static void* NEXT(void* bp) {return (void*)(uint32_t*)bp+1;};

//4,8,16,32,64,128,256,512,1024,2048,4096
void* sizes[11];

void docSize(void* bp) {
    uint32_t index = (uint32_t)(log2(GET_SIZE(HDRP(bp))/WSIZE)) - 2;
    if (index > 10) {
        index = 10;
    }
    if (sizes[index] == NULL) {
...
void remSize(void* bp) {
    ...
    if (NEXT(bp) != NULL) {
        //next->prev = prev
        PUT(PREV(*(void**)NEXT(bp)),GET(PREV(bp)) );
    }
    if (PREV(bp) != NULL) {
        //next->prev = next
        PUT(NEXT(*(void**)PREV(bp)),GET(NEXT(bp)) );
    }
}
...
static void *find_fit(uint32_t asize)
{
    uint32_t index = (uint32_t)(log2(asize/WSIZE)) - 2;
    if (index > 10) {
        index = 10;
    }
    //goes through each element in index, if none are found searches bigger indices
    for (int i=index;i<11;i++) {
        for (void *temp = sizes[i];temp != NULL;temp = *(void**)temp) {
...
static void *coalesce(void *bp) 
{
    //ftr and hdr should return the same result
    ...
    if (prev_alloc && !next_alloc) {
        ...
        PUT(HDRP(bp),PACK(size,0));
        PUT(FTRP(bp),PACK(size,0));
        remSize(NEXT_BLKP(bp));
    } else if (!prev_alloc && next_alloc) {
        size += GET_SIZE(HDRP(PREV_BLKP(bp)));
        PUT(FTRP(bp),PACK(size,0));
        PUT(HDRP(PREV_BLKP(bp)),PACK(size,0));
        remSize(PREV_BLKP(bp));
        bp = PREV_BLKP(bp);
    } else if (!prev_alloc && !next_alloc) {
...
void *mm_malloc(uint32_t size) 
{
    ...
    if (size <= DSIZE) {
        ...
        asize = DSIZE + DSIZE * ( (size + DSIZE + (DSIZE-1)) / DSIZE );
    }
...
void mm_checkseg() {
    for (int i=0;i<11;i++) {
        void* curr = sizes[i];
        printf("%d, %p, %d ",i,sizes[i],curr == NULL);
        while(1) {
            if (curr == NULL) {
                printf("NULL");
                break;
            } else {
                printf("%d, ",GET_SIZE(HDRP(curr) ) );
            }
            curr = *(void**)curr;
        }
        printf("\n");
