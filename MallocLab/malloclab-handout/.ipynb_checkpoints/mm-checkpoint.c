...
#define BSIZE       12      //size of a binary tree block with 0 bytes payload
...
static void* GET_LEFT(void* bp) {return (void*)*((uint32_t*)bp);}
static void* GET_RIGHT(void* bp) {return (void*)((uint32_t*)bp+1);}
...
static void *find_fit(uint32_t asize)
{
    ...
    while (asize > GET_SIZE(HDRP(root))) {
        temp = GET_RIGHT(root);
    }
    if (GET(temp) == 0) {
        return NULL;
...
static void *coalesce(void *bp) 
{
    //ftr and hdr should return the sam result
    ...
    //if you need to change the current block, delete it before modifying
    delete(bp);
    if (prev_alloc && !next_alloc) {
        delete(next_alloc);
...
void *mm_malloc(uint32_t size) 
{
    ...
    if (size <= DSIZE) {
        ...
        asize = WSIZE + DSIZE * ( (size + DSIZE + (DSIZE-1)) / DSIZE );
    }
